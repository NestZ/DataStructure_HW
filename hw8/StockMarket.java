...
public class StockMarket {
    ...
    public void showStockOwnership(){
        ...
        for (int i = 0; i < numInvestor; i++) {
            ...
            for (int j = 0; j < numStock; j++) {
                System.out.print(stockOwnership[i][j]);
                if (j<numStock-1)
                    System.out.print(",");
            }
            System.out.print("},\n");
    ...
    public void submitBuyOrder(String investorName, String stockName, int amount, double price){
        ...
        if (price > 0) {
            if (buyerQueue[stockID] == null) {
                ...
                // limit number of buy orders = 20 per stock 
    ...
    public void matching(char orderCode, int stockID){
        // check the matching only if there is at least one seller and one buyer
